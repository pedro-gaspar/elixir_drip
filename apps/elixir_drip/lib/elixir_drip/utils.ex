defmodule ElixirDrip.Utils do
  ...
  def ast_without_metadata({op, _metadata, value}) when is_tuple(value),
    do: {op, ast_without_metadata(value)}

  def ast_without_metadata({op, _metadata, list}) when is_list(list),
    do: {op, ast_without_metadata(list)}

  def ast_without_metadata([value | rest]),
    do: [ast_without_metadata(value)] ++ ast_without_metadata(rest)
  ...
  defmodule FillCache do
    alias ElixirDrip.Storage.Supervisors.SlowCacheSupervisor, as: Cache

    def fill(how_many, offset) do
      offset..(offset + (how_many - 1))
      |> Enum.each(&Cache.put(to_string(&1), "Media content #{&1}."))
    end
  end
end

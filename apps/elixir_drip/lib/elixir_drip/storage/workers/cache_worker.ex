defmodule ElixirDrip.Storage.Workers.CacheWorker do
  ...
  def name_for(media_id), do: {:via, Registry, {ElixirDrip.Registry, {:cache, media_id}}}
  ...
  def handle_info(:expire, %{hits: hits} = state) do
    Logger.debug(
      "#{inspect(self())}: Terminating process... Served the cached content #{hits} times."
    )

    {:stop, :normal, state}
  end

  def handle_info(msg, state), do: IO.inspect({msg, state}, label: "Unhandled message")
  ...
  def handle_call(:get_media, _from, %{hits: hits, content: content, timer: timer} = state) do
    ...
    Logger.debug(
      "#{inspect(self())}: Received :get_media and served '#{inspect(content)}' bytes #{hits + 1} times."
    )
  ...
  defp refresh_timer(timer) do
    ...
    Logger.debug(
      "#{inspect(self())}: Canceled the previous expiration timer. Will now expire in #{expires_in} milliseconds."
    )
